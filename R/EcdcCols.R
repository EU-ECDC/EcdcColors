...
#' EcdcCols("red", n=7)
#' 
#' EcdcCols("grey", grey_shade = c("mediumlight", "dark"))
#' 
#' # Use in a graph
#' # Dummy data
#' mydat <- data.frame(ID = c(seq(1,10,1)),
#' Gender = c(rep(c("F", "M"),5)))
#' barplot(table(mydat$Gender), col = EcdcCols(col_scale = "qual", n=2))
#' 
#' # Hot-cold colour scale
#' barplot(c(1:4), col = EcdcCols(col_scale = "hotcold", n = 4, hot_cols = 2))
EcdcCols <- function(col_scale = "green", n = NULL, grey_shade = NULL,
                       hot_cols = NULL){
